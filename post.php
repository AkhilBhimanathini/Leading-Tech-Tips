...
?>
								<div class="post_share ml-sm-auto">
									...
									<ul class="post_share_list">
										<li class="post_share_item"><a href=""><i class="" aria-hidden="true">
                      <?php
echo '<iframe src="https://www.facebook.com/plugins/share_button.php?href='.$actual_link.'&layout=button_count&size=large&mobile_iframe=true&width=83&height=28&appId" width="83" height="28" style="border:none;overflow:hidden padding-top:10px;" scrolling="no" frameborder="0" allowTransparency="true" ></iframe>';
?> 
          ...
          ?>
			...
			<div class="row">
               ...
               <?php
                      $get = "select post_tag from post where post_id='".$_GET['post_id']."' and  (not post_id='".$_GET['post_id']."')  ";
          ...
          <div id="success-message"></div>
					...
					else
					{
					 ...
					 <div class="post_comment">
						<div class="post_comment_title">Post Comment</div>
						<div class="post_comment_form_container"><h1>Hay! <?php echo $_SESSION['username'] ; ?></h1>
						<form id="addCommUser" >
                        <textarea id="comment-user" class="comment_text" placeholder="Your Comment" required="required" name="comment"></textarea>
                        <input type="submit" id="save-comment-user" class="comment_button" name="submit" placeholder="Post Comment">
            </form>
						</div>
					</div>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $("#save-comment").on("click",function(e){
      ...
      $("#save-comment-user").on("click",function(e){
      e.preventDefault();
      ...
      if(comment == "")
      {
        $("#error-message").html("Please Comment").slideDown();
        $("#success-message").slideUp();
      }
      else{
      $.ajax({
        url : "php/post-comment-user.php",
        type : "POST",
        data : {name : name,comment : comment, post_id:post_id},
        success : function(data){
          if(data ==1){
            $("#addCommUser").trigger("reset");
            $("#success-message").html("Comment Posted.").slideDown();
            $("#error-message").slideUp();
            loadTable();
          }
